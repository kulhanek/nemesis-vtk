...
QObject* XYZAxesSetupCB(void* p_data);

CExtUUID        XYZAxesSetupID(
                    "{XYZ_AXES_SETUP:7724c44e-bc09-4da8-bd78-1a263efcda20}",
                    "XYZ Axes");

CPluginObject   XYZAxesSetupObject(&NemesisCorePlugin,
                    XYZAxesSetupID,GRAPHICS_SETUP_CAT,
...
QObject* XYZAxesSetupCB(void* p_data)
{
    return(new CXYZAxesSetup(static_cast<CProObject*>(p_data)));
}
...
CXYZAxesSetup::CXYZAxesSetup(CProObject* p_owner)
    : CGraphicsSetup(&XYZAxesSetupObject,p_owner)
{
    PXColor.SetRGB(0.8,0,0,1);
    NXColor.SetRGB(0.8,0,0,1);
    ConeXColor.SetRGB(0.8,0,0,1);

    PYColor.SetRGB(0,0.8,0,1);
    NYColor.SetRGB(0,0.8,0,1);
    ConeYColor.SetRGB(0,0.8,0,1);

    PZColor.SetRGB(0,0,0.8,1);
    NZColor.SetRGB(0,0,0.8,1);
...
void CXYZAxesSetup::LoadData(CXMLElement* p_ele)
{
    if( p_ele == NULL ) {
    ...
    p_ele->GetAttribute("sx",ShowX);
    p_ele->GetAttribute("sy",ShowY);
    p_ele->GetAttribute("sz",ShowZ);

    p_ele->GetAttribute("smx",ShowMX);
    p_ele->GetAttribute("smy",ShowMY);
    p_ele->GetAttribute("smz",ShowMZ);

    p_ele->GetAttribute("mx",MagnificationX);
    p_ele->GetAttribute("my",MagnificationY);
    p_ele->GetAttribute("mz",MagnificationZ);

    p_ele->GetAttribute("mmx",MagnificationMX);
    p_ele->GetAttribute("mmy",MagnificationMY);
    p_ele->GetAttribute("mmz",MagnificationMZ);
}

//---------------------------------------------------------------------------

void CXYZAxesSetup::SaveData(CXMLElement* p_ele)
{
    if( p_ele == NULL ) {
        INVALID_ARGUMENT("p_ele is NULL");
    }
    ...
    p_ele->SetAttribute("sx",ShowX);
    p_ele->SetAttribute("sy",ShowY);
    p_ele->SetAttribute("sz",ShowZ);

    p_ele->SetAttribute("smx",ShowMX);
    p_ele->SetAttribute("smy",ShowMY);
    p_ele->SetAttribute("smz",ShowMZ);

    p_ele->SetAttribute("mx",MagnificationX);
    p_ele->SetAttribute("my",MagnificationY);
    p_ele->SetAttribute("mz",MagnificationZ);

    p_ele->SetAttribute("mmx",MagnificationMX);
    p_ele->SetAttribute("mmy",MagnificationMY);
    p_ele->SetAttribute("mmz",MagnificationMZ);
}
