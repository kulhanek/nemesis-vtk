...
QObject* PdbExportJobCB(void* p_data);
...
CPdbExportJob::CPdbExportJob(CStructure* p_mol,const QString& name)
    : CJob(&PdbExportJobObject,p_mol->GetProject())
{
    Structure = p_mol;
    FileName = name;
...
bool CPdbExportJob::JobAboutToBeSubmitted(void)
{
...
bool CPdbExportJob::InitializeJob(void)
{
...
bool CPdbExportJob::ExecuteJob(void)
{
    Tick = 1;
...
bool CPdbExportJob::FinalizeJob(void)
{
    // close file
    sout.close();
...
void CPdbExportJob::WriteHead(CStructure* p_mol)
{
    // nothing to do here
    // reserved for future
}
...
void CPdbExportJob::WriteAtoms(CAtomList* p_atoms)
{
...
void CPdbExportJob::InitMoleculeIndexes(CStructure* p_mol)
{
