...
#include "DockingProjectReceptorImportJob.moc"
...
QObject* DockingProjectReceptorImportJobCB(void* p_data);

CExtUUID        DockingProjectReceptorImportJobID(
                    "{DOCKING_PROJECT_RECEPTOR_IMPORT_JOB:9a64857d-8e60-4ae5-965b-85eac7baaec7}",
                    "DockingProject Receptor Import");

CPluginObject   DockingProjectReceptorImportJobObject(&DockingProjectPlugin,
                    DockingProjectReceptorImportJobID,JOB_CAT,
                    ":/images/OpenBabel/Openbabel.png",
                    DockingProjectReceptorImportJobCB);
...
QObject* DockingProjectReceptorImportJobCB(void* p_data)
{
    ...
    if( p_project == NULL ){
        ES_ERROR("CDockingProjectReceptorImportJob requires active project");
        return(NULL);
    }

    QObject* p_importjob = new CDockingProjectReceptorImportJob(p_project);
    return(p_importjob);
}
...
CDockingProjectReceptorImportJob::CDockingProjectReceptorImportJob(CProject* p_project)
    : CImportJob(&DockingProjectReceptorImportJobObject,p_project)
{
...
bool CDockingProjectReceptorImportJob::JobAboutToBeSubmitted(void)
{
...
bool CDockingProjectReceptorImportJob::InitializeJob(void)
{
    ...
    if( p_docproj != NULL ){
        ...
        p_docproj->SetReceptorFileName(file);

        // following change is composite
        // it is composed from EHCL_TOPOLOGY and EHCL_GRAPHICS
        p_history = Structure->BeginChangeWH(EHCL_COMPOSITE,"import Receptor " + name + " structure");
        ...
        // following change is composite
        // it is composed from EHCL_TOPOLOGY and EHCL_GRAPHICS
        p_history = Structure->BeginChangeWH(EHCL_COMPOSITE,"import Receptor structure");
        if( p_history == NULL ) return(false);
...
bool CDockingProjectReceptorImportJob::ExecuteJob(void)
{
...
bool CDockingProjectReceptorImportJob::FinalizeJob(void)
{
    // THREAD SAFETY !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // do some magic with parent and thread ownership
