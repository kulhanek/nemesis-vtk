    ...
    : CWorkPanel(&SuperCellBuilderWorkPanelObject,p_pro,EWPR_TOOL)
{
    ...
    connect(WidgetUI.repeatCSB,SIGNAL(valueChanged(int)),
            this,SLOT(StructureChanged()));
    connect(WidgetUI.buildSuperCellPB,SIGNAL(clicked(bool)),
            this, SLOT(BuildSuperCell(void)));
    if( GetProject() ){
...
{
    bool set = true;
    set &= WidgetUI.structureW->getObject() != NULL;
    set &= (WidgetUI.repeatASB->value() > 1) || (WidgetUI.repeatBSB->value() > 1) || (WidgetUI.repeatCSB->value() > 1);
    WidgetUI.buildSuperCellPB->setEnabled(set);
}

//------------------------------------------------------------------------------

void CSuperCellBuilderWorkPanel::BuildSuperCell(void)
{
